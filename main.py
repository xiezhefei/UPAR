...
def create_model( session ):
    # Create
    model = UPAR( Pmax=Pmax, var=var, feature_dim=feature_dim, batch_size=batch_size, layers=layers, learning_rate=learning_rate)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        # Unrolled WMMSE experiment
            ...
            if mode == 'train':
                ...
                for epoch in range(nEpoch):
                    ...
                    if train_rate > max_rate:
                        max_rate = train_rate
                        model.save(sess, path='models/'+dataID+'/upar-model', global_step=(epoch+1))
            ...
            for batch in range(test_iter):
                ...
                Batch_rate = np.append(Batch_rate, batch_rate)
            ...
            test_rate /= test_iter
            #np.save('data/' + dataID + '/Hegrnwmmse.npy', Batch_rate)
